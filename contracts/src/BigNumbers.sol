...
library BigNumbers {
    ...
    function _modexp(
        ...
        assembly {
            ...
            ///the following code removes any leading words containing all zeroes in the result.
            let result_data_ptr := msword_ptr         // Start of precompile result data
            let result_length := ml                   // Correct length

            // Pointer where the length word should reside (32 bytes before data starts)
            let r_ptr := sub(result_data_ptr, 0x20)

            // Store the correct length 'ml' at this location
            mstore(r_ptr, result_length)

            // Assign this pointer 'r_ptr' (which now points to a valid bytes structure)
            // to the return variable 'r'.
            r := r_ptr

            // Update the free memory pointer to be past the precompile's output buffer.
            // Use the original logic, assuming 'freemem' is the start of the precompile input buffer.
            mstore(0x40, add(add(96, freemem), result_length)) // Use correct length 'ml' (result_length)
        }        
    }
