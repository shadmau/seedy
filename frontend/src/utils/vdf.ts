...
console.log("--- Real VDF Utils Loaded ---");
...
export async function evaluateVDF(x: bigint, TInput: bigint | number, N: bigint): Promise<bigint> {
    const T = BigInt(TInput); // Ensure T is bigint
    ...
    if (T === BigInt(0)) {
        ...
        return x; // Base case: T=0
    }

    let y = x;
    const tNum = Number(T); // Use Number for loop counter, check safety
    if (!Number.isSafeInteger(tNum)) {
        console.warn(`[VDF Debug] evaluateVDF: T=${T} may be too large for JS loop counter.`);
        // Potentially throw error or use different loop strategy for huge T
    }
    console.log(`[VDF Debug] evaluateVDF: Starting loop for ${tNum} iterations.`);
    // Adjust log interval calculation to avoid division by zero if tNum is small
    const logInterval = tNum > 0 ? Math.max(1, Math.floor(tNum / 10)) : 1; // Log roughly 10 times

    for (let i = 0; i < tNum; i++) {
        try {
             // Use modPow for potentially better performance/safety with large numbers
            y = modPow(y, BigInt(2), N);
        } catch (e) {
            console.error(`[VDF Debug] evaluateVDF: Error in modPow at iteration ${i}, y=${y}, N=${N}`, e);
            throw e;
        }
        if ((i + 1) % logInterval === 0 || i === tNum - 1) { // Log progress
            // Limit length of logged y for readability
...
export async function generateProof(xInput: `0x${string}` | bigint, TInput: bigint | number, deltaInput: bigint | number, NInput: `0x${string}` | bigint) {
    ...
    if (TNum <= 0) { // Check TNum after conversion
    ...
    // --- Proof Generation Loop (Wesolowski) ---
    ...
    for (let i = 1; i <= loopEnd; i++) { 
        ...
        // Calculate the number of squarings needed
        ...
        // *** Correct vi calculation using repeated squaring loop ***
        let vi = xi; // Start with xi
        const iterNum = Number(iterations); // Use Number for loop counter
        ...
        try {
            ...
            ri = hashBigIntsEthersStyle(xi, yi, vi);
    ...
    console.log(`[VDF Debug] generateProof: Finished proof loop. Final proof array (length ${proof.length}): [${proof.map(p => p.toString().substring(0,10) + '...').join(', ')}]`); // Log truncated proof
    return { y, proof };
} 